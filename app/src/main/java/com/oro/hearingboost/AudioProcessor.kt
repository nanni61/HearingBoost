...
class AudioProcessor {
    ...
    // ── High-Pass ─────────────────────────────────────────────────────────────
    @Volatile var hpEnabled = false
    @Volatile var hpFreqHz  = 80.0
    ...
    private val bpLow  = BiquadFilter()
    private val bpHigh = BiquadFilter()
    ...
    fun start() {
        ...
        job = scope.launch {
            ...
            val outBuf = FloatArray(BUFFER_FRAMES * 2)
            var smoothRms = 0.0f
            ...
            while (isActive) {
                ...
                for (i in 0 until read) {
                    val gated = if (abs(inBuf[i]) < noiseThresh) inBuf[i] * 0.05f else inBuf[i]
                ...
                if (gateEnabled) {
                    ...
                    for (i in 0 until read) {
                        if (abs(inBuf[i]) < threshLinear) inBuf[i] = 0f
                    }
                }
                ...
                var j = 0
                for (i in 0 until read) {
                    outBuf[j++] = softClip(inBuf[i] * gl)
                    outBuf[j++] = softClip(inBuf[i] * gr)
                }
